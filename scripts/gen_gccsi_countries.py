#!/usr/bin/env python3
"""Regenerate dashboard/data/curation/gccsi-countries.csv from the GCCSI
...
def main():
    ...
    fields = ["country", "operating", "construction", "advanced_development",
              "early_development", "pipeline", "total_facilities",
              "cross_border_participation", "capacity_mtpa",
              "capacity_all_stages_mtpa", "page", "policy_status", "policy_page",
              "policy_note", "carbon_price", "carbon_price_page"]
    path = _curation.write_rows(OUT_CSV, fields, out_rows)

    print(f"\nwrote {path}")
    print(f"  {len(out_rows)} countries · {len(rows)} facilities parsed")
    if problems:
        print("  ! validation mismatches:", "; ".join(problems), file=sys.stderr)
    print("\n  Caveats and known gaps for this dataset live in "
          "dashboard/data/curation/NOTES.md — update them there if this run "
