...
- `weekly-audit.yml` — cron 08:00 Sat Melbourne. Renders `audit/${SAT}-recall-report.{html,md}`, rebuilds the dashboard, and dispatches both email workflows. The suite runs on **both sides** of the rebuild — once against the committed inputs, once against the freshly regenerated `dashboard/data` — so a data-integrity guard blocks the weekly commit rather than shipping a wrong dashboard. Gating only before the rebuild was not enough: that pass runs against data the rebuild is about to overwrite, so a bad derived figure was committed by the very run that produced it and went red only the *following* Saturday (2026-08-25). A `notify-failure` job emails an alert (naming the failing tests) when a *scheduled* run fails — without it the whole chain fails silently, as it did for three consecutive Saturdays from 2026-08-08 to -22. Manual: Actions tab → Weekly recall audit → Run workflow.
